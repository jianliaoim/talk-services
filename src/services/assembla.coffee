...
_apiHost = 'https://api.assembla.com'
_pageHost = 'https://www.assembla.com'

###*
 * Create assembla hook
...
###*
 * Remove assembla hook
 * @param  {String} repos - Repos name
 * @param  {String} hookId - The assembla hook id
 * @param  {String} token - Assembla user token
 * @return {Promise}
###
...
###*
 * Update assembla hook
 * @param  {String} repos - Repos name
 * @param  {String} hookId - Assembla hook id
 * @param  {String} token - Token
 * @param  {Array} events - Events of hook
 * @param  {String} hashId - hashId of integration
 * @return {Promise} Response body
...
_updateWebhook = (integration) ->
  ...
  .map (repos) ->
    ...
    if (repos in _original.repos) or not integration.isDirectModified 'repos'
      ...
      hookId = data[_repos]?.hookId
      throw new Error('Assembla hook not found') unless hookId  # Stop the update process when hookId not found
      _updateHook repos
      , hookId
      , integration.token
      , integration.events
...
_receiveWebhook = ({headers, body, integration}) ->
  self = this
  event = headers['x-assembla-event']
  payload = body
...
module.exports = service.register 'assembla', ->

  @title = 'assembla'

  @summary = service.i18n
    zh: '分布式的版本控制系统。'
    en: 'assembla offers online source code hosting for Git projects.'

  @description = service.i18n
    zh: 'assembla 是一个分布式的版本控制系统。选择一个话题添加 assembla 聚合后，你就可以在被评论、创建或删除分支、仓库被 fork 等情形下收到简聊通知。'
    en: 'assembla offers online source code hosting for Git projects. This integration allows you receive assembla comments, pull request, etc. '

  @iconUrl = service.static 'images/icons/assembla@2x.png'
